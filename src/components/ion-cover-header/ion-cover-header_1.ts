...
@Directive({ 
  selector: '[ion-cover-header-1]',
})
export class IonCoverHeader_1 {
  // inputs
  @Input('ion-cover-header-1') opaqueAt: number; // the value in pixels where the bar-header should be totaly opaque
  // internals
  ...
  ngOnInit() {
    ...
    this.toolbarElement = this.queryElement(this.ionHeaderElement, 'div');
    if (this.toolbarElement) {
      this.toolbarBackgroundElement = this.queryElement(this.toolbarElement, '.header-image');
      this.toolbarContentElement = this.queryElement(this.toolbarElement, '.header-content');
    } else {
